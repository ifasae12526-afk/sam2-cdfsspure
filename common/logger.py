...
class AverageMeter:
    ...
    def update(self, inter_b, union_b, class_id, loss):
        # class_id harus 1D vector untuk index_add_
        # kemungkinan bentuk: scalar [], [B], atau [B,1]
        if isinstance(class_id, torch.Tensor):
            class_id = class_id.view(-1).long()   # <-- ini kuncinya
        else:
            class_id = torch.tensor([class_id], device=inter_b.device).long()
    ...
    def write_result(self, split, epoch):
        ...
        if len(self.loss_buf) > 0:
            ...
            avg_loss = 0.0  # dataloader kosong / tidak ada update
    ...
    def write_process(self, batch_idx, datalen, epoch, write_batch_idx=20):
        if batch_idx % write_batch_idx == 0:
            ...
            iou, fb_iou = self.compute_iou()
            if epoch != -1 and len(self.loss_buf) > 0:
                loss_buf = torch.stack(self.loss_buf)
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        logpath = args.logpath if training else '_TEST_' + os.path.basename(os.path.dirname(args.load)).split('.')[0] + logtime
