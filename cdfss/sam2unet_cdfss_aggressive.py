...
def _resize_mask(mask: torch.Tensor, size_hw: Tuple[int, int]) -> torch.Tensor:
    """
    Resize binary mask to match a feature map spatial size.
    Uses bilinear then ( > 0 ) binarization to avoid empty masks at coarse levels.
    """
    if mask.dim() == 3:
    ...
    # key change: bilinear keeps fractional coverage for tiny objects
    mask_rs = F.interpolate(mask, size=size_hw, mode="bilinear", align_corners=False)
    mask_rs = (mask_rs > 0.0).float()   # keep it binary, compatible with other code paths
    return mask_rs
...
class PATAnchorTransform(nn.Module):
    ...
    def _compute_P(
        ...
        Uses a stable closed-form pseudoinverse for 2xD matrices:
            pinv(C) = C^T (C C^T + λI)^(-1)
        """
        b, d = proto_fg.shape
        device = proto_fg.device
        out_dtype = proto_fg.dtype  # keep output dtype consistent with the rest of the forward

        # Build C and R in fp32 for numerical stability (esp. under AMP/autocast)
        C = torch.stack([proto_bg, proto_fg], dim=1).float()  # (B,2,D)
        R = self.reference_layers[level_idx].weight.unsqueeze(0).expand(b, -1, -1).float()  # (B,2,D)

        # Normalize rows
        C = torch.nn.functional.normalize(C, dim=2, eps=eps)
        R = torch.nn.functional.normalize(R, dim=2, eps=eps)

        # Gram matrix (B,2,2)
        G = C @ C.transpose(1, 2)

        # Ridge regularization to avoid singularity when fg/bg prototypes are degenerate/collinear
        ridge = 1e-3
        I2 = torch.eye(2, device=device, dtype=G.dtype).unsqueeze(0)
        G = G + ridge * I2
        ...
        # Match original implementation: transpose at the end
        P = P.transpose(1, 2).contiguous()  # (B,D,D)

        # Optional but highly recommended: clamp extremely large transforms
        # Identity Frobenius norm ~ sqrt(D). For D=256, ~16.
        # If P becomes huge, it will blow up activations.
        max_frob = 100.0
        frob = P.pow(2).sum(dim=(1, 2)).sqrt()  # (B,)
        scale = (frob / max_frob).clamp(min=1.0).view(b, 1, 1)
        P = P / scale

        # Safety: if anything is non-finite, fall back to identity
        if not torch.isfinite(P).all():
            bad = ~torch.isfinite(P).view(b, -1).all(dim=1)
            if bad.any():
                P[bad] = torch.eye(d, device=device, dtype=P.dtype)
